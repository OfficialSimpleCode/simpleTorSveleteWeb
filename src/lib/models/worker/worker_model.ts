...
export default class WorkerModel {
    ...
    //------------------------------ listners -------------------------------
    workerDocListner?: StreamSubscription<
    ...
    static fromWorkerDocJson(
      workerJson: { [key: string]: any }
    ): WorkerModel {
      const workerModel = new WorkerModel({});
    ...
    shiftsFor({ day }: { day: Date }): string[] {
      // regular shifts
      const defaultShifts =
        this.workTime[day.toLocaleString("en-US", { weekday: "long" }).toLowerCase()] || [];
      // specific changes
      for (const specifiRangeObj of Object.values(this.specificRangeChanges)) {
        if (
          durationStrikings(
            ...
            new Date(
              specifiRangeObj.end.getTime() + 23 * 60 * 60 * 1000 +
                59 * 60 * 1000
            ),
            day,
            day
    ...
    setFromWorkerDoc(workerJson: { [key: string]: any }): void {
      ...
      if (workerJson["lastCleanDate"] instanceof Timestamp) {
        ...
        this.lastCleanDate =
          new Date(workerJson["lastCleanDate"]) || new Date();
      }
      ...
      if (workerJson["devices"] != null && typeof workerJson["devices"] === "object") {
      ...
      if (workerJson["notifications"]) {
        ...
        this.notifications = new WorkerNotificatiosSettings(
          ...
          {
            [BookingReminderType.regular]: workerJson["minutesBeforeNotify"] || 60
          },
    ...
    addOrUpdateMultiBookingInBookingsObjects(multiBooking: MultiBooking): void {
      ...
      this.bookingObjects[dateStr] ||= new BookingsDay(dateStr);
      this.bookingObjects[dateStr].multiBookings[
        multiBooking.bookingId
      ] = multiBooking;
    }
    // Update booking data
function removeMultiBookingInBookingsObjects(multiBooking) {
  ...
  // Update booking data
  function removeBookingInBookingsObjects(booking) {
  ...
  function setBookingsObjects(bookingsObjectsJson, dateString) {
    /* This func only for the worker himself - only he get 
      the bookingsObjects collection*/
    bookingObjects[dateString] = { dateStr: dateString };
    Object.entries(bookingsObjectsJson).forEach(([bookingId, bookingJson]) => {
      const bookingObj = Booking.fromJson(bookingJson, bookingId);
      bookingObjects[dateString].bookings[bookingObj.bookingId] = bookingObj;
    });
  }
  // initial the var treatmentsById by the current treatmentsSubjects
  function initialTreatmentsById() {
    treatmentsById = {};
    // initial the treatmentById map
    Object.values(treatmentsSubjects).forEach((subject) => {
      Object.values(subject.treatments).forEach((tempTreatment) => {
        treatmentsById[tempTreatment.id] = tempTreatment;
      });
    });
  }
  function getInvoiceWorkerInfo() {
    return InvoiceWorkerInfo({ workerId: id, workerName: name });
  }
  function getFcmsTokens() {
    if (!allowAppNotification) {
      return new Set();
    }
    if (tempFcm != null) {
      return new Set([tempFcm]);
    }
  ...
  // --------------- Actions To The DB ------------------
  function toWorkerDocJson() {
    const data =  new Record<string, any>();
    data['workTime'] = {};
    Object.entries(workTime).forEach(([day, list]) => {
      data['workTime'][day] = list;
    });
    data['specificRangeChanges'] = {};
    Object.entries(specificRangeChanges).forEach(([rangeKey, shiftChangeRange]) => {
    ...
    if (closeCalendarDate != null) {
      // the format is not the regular its cant
      // be changed - already used in production
      data['closeCalendarDate'] = new Intl.DateTimeFormat('en-US', { day: '2-digit', month: '2-digit', year: 'numeric' }).format(closeCalendarDate);
    }
    if (minDateForShowinData.year != new Date(0).getFullYear()) {
      data['minDateForShowinData'] = dateToDateStr(minDateForShowinData);
    }
    if (!openCalendar) {
      data['openCalendar'] = openCalendar;
    }
    data['allowBookingConfirmations'] = allowBookingConfirmations;
    if (!mustVerifyPhone) {
      data['mustVerifyPhone'] = mustVerifyPhone;
    }
    if (isCustomersNeedRecurrence) {
      data['isCustomersNeedRecurrence'] = isCustomersNeedRecurrence;
    }
    data['weekendDays'] = weekendDays;
    data['maxTreatments'] = maxTreatments;
    data['gender'] = genderToStr[gender];
    data['onHoldMinutes'] = onHoldMinutes;
    data['cancelMinutes'] = cancelMinutes;
    if (hasRecurrenceEvents) {
      data['hasRecurrenceEvents'] = hasRecurrenceEvents;
    }
    if (!showVacations) {
      data['showVacations'] = showVacations;
    }
    data['devices'] = {};
    Object.entries(devices).forEach(([fcmId, fcm]) => {
      data['devices'][fcmId] = fcm.toJson();
    });
    data['deleteNeerDedlineBookingMessage'] = deleteNeerDedlineBookingMessage;
    data['orderNeerDedlineBookingMessage'] = orderNeerDedlineBookingMessage;
    data['generalBookingMessage'] = generalBookingMessage;
    data['closeScheduleOnHolidays'] = closeScheduleOnHolidays;
    if (customWindowMinute != 30) {
      data['customWindowMinute'] = customWindowMinute;
    }
    if (windowSpaces != WindowSpaces.shortestTreatment) {
      data['windowSpaces'] = windowSpacesToStr[windowSpaces];
    }
    if (minutesBeforeBookingToConfirm != 1440) {
      data['minutesBeforeBookingToConfirm'] = minutesBeforeBookingToConfirm;
    }
    data['bookingDaysToDeleteLocal'] = [];
    data['notifications'] = notifications.toJson();
    data['lastCleanDate'] = lastCleanDate.toString();
    data['daysToAllowBookings'] = daysToAllowBookings;
    if (maxFutureBookings != 4) {
      data['maxFutureBookings'] = maxFutureBookings;
    }
    if (currentClientCount != 0) {
      data['currentClientCount'] = currentClientCount;
    }
    if (!allowAppNotification) {
      data['allowAppNotification'] = allowAppNotification;
    }
    if (waitingListExceptionDays.length > 0) {
      data['waitingListExceptionDays'] = Array.from(waitingListExceptionDays);
    }
    data['treatmentsSubjects'] = {};
    Object.entries(treatmentsSubjects).forEach(([index, treatmentsSubject]) => {
      data['treatmentsSubjects'][index] = treatmentsSubject.toJson();
    });
    if (!showWaitingListWhenThereIsChoises) {
      data['showWaitingListWhenThereIsChoises'] = showWaitingListWhenThereIsChoises;
    }
    if (isVerifiedPhone) {
      data['isVerifiedPhone'] = isVerifiedPhone;
    }
    ...
    data['id'] = id;
    data['storyImages'] = {};
    Object.entries(storyImages).forEach(([imageId, image]) => {
  ...
  function toString() {
    return JSON.stringify(toWorkerDocJson()) + workerPublicData.toWorkerPublicDataJson().toString();
  }
