class MultiBookingUser {
    ...
    constructor(params: {
      ...
      workerRemindersTypes?: Map<BookingReminderType, number>,
      customerId?: string,
      orderingOptions?: OrderingOptions,
      remindersTypes?: Map<BookingReminderType, number>,
      isUserExist?: boolean,
      ...
      userFcms?: Set<string>,
      clientNote?: string,
      invoices?: Map<string, BookingInvoiceData>,
      transactions?: Map<string, BookingTransactionModel>,
      ...
      this.workerNotificationOption = params.workerNotificationOption || NotificationOption.PushOrSMS;
    ...
    copyDataToOrder(params: {
      ...
      worker.notifications.remindersTypes.forEach((type, minutes) => {
        ...
        if (type === BookingReminderType.confirmArrival && this.confirmedArrival) {
          return;
        }
        if (
          bookingDate.getTime() - minutes * 60 * 1000 <
          Date.now() + 1000
        ) {
          this.remindersTypes.set(
            ...
            Math.abs(
              Math.floor(
                (Date.now() - bookingDate.getTime()) / 1000 / 60 / 2
              )
            ) - ((Math.abs(Math.floor((Date.now() - bookingDate.getTime()) / 1000 / 60 / 2))) % 5)
          );
      ...
      if (isWeb) {
        ...
        this.orderingOptions = OrderingOptions.app;
      }
      ...
      if (!workerAction) {
        ...
        this.addToClientAt =
          ...
          !clientAt.get(BusinessInitializer().business.businessId)!.has(worker.id);
      } else {
    ...
    get toBooking(): Booking {
      ...
      return {
        ...
        .workerRemindersTypes = new Map(this.workerRemindersTypes)
    ...
    static fromJson(
      ...
      const multiBookingUser = new MultiBookingUser({
        ...
        remindersTypes: json["remindersTypes"]
          ...
          : new Map([[BookingReminderType.regular, json["minutesBeforeNotify"] || 60]]),
        workerRemindersTypes: json["workerRemindersTypes"]
          ? new Map(
              Object.entries(json["workerRemindersTypes"]).map(([type, minutes]) => [
                bookingReminderTypeFromStr[type],
                minutes,
              ])
            )
        ...
        status: bookingsMassageKeys[json['status'].toString()] || BookingStatuses.approved,
        ...
        notificationType:
          notificationTypeFromStr[json["notificationType"]] || NotificationType.none,
        workerNotificationOption:
          notificationOptionFromStr[json["workerNotificationOption"]] || NotificationOption.PushOrSMS,
        ...
        transactions: json["transactions"]
          ? new Map(
              Object.entries(json["transactions"]).map(([id, transactionJson]) => [
                id,
    ...
    toJson(): any {
      ...
      this.workerRemindersTypes.forEach((reminder, minutes) => {
        data['workerRemindersTypes'][bookingReminderTypeToStr[reminder]] = minutes;
      });
      if (this.userDeleted) {
