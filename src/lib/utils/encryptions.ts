...
const maxAttemptsPassword = 3; // Replace this with your actual max attempts value

function encryptText({ text, password }: { text: string; password: string }): string {
...
function decryptText({ encryption, password }: { encryption: string; password: string }): string {
  const key = generateKey(password);
  const iv = enc.Utf8.parse(iv16len);
...
function generateKey(password: string): enc.WordArray {
  ...
  preKey = SHA256(password + preKey).toString(enc.Hex).substring(0, 32);

  return enc.Utf8.parse(preKey);
}

function hashText({ text, salt = 'simpleCode' }: { text: string; salt?: string }): string {
  if (text === '') {
    return '';
  }
...
function hashId({ id }: { id: string }): string {
  ...
  return hashValue.substring(0, Math.min(10, hashValue.length)).replace(/[/.*]/g, '');
}

function isVerifiedPasswordForHash({ hash, password }: { hash: string; password: string }): boolean {
  const passwordHash = SHA256(password + 'simpleCode', 5000).toString(enc.Hex);
  return passwordHash === hash;
}

function shortHashText({ text, salt = 'a' }: { text: string; salt?: string }): string {
  if (text === '') {
    return '';
  }
