 ...
 */
function relevantMultiEventTime({
  worker,
  dateForCheck,
  treatment,
}: {
 ...
 */
function isAllDayTaken(worker: WorkerModel, date: Date, workerAction: boolean, workerSheet: boolean): boolean {
  ...
  if (!workerAction && worker.recurrence.vacationInDate(date) !== null) {
    logger.d("Vacation is a free day for this worker -> don't generate times");
    return true;
  }
 ...
 */
function getJump(currentTime: Date, forbbidenTimes: Date[], indexesInForbidden: number[], worker: WorkerModel): number {
  const defaultAdding =
 ...
 * @returns The calculated jump time.
 */
function getJumpReversed(currentTime: Date, forbbidenTimes: Date[], indexesInForbidden: number[], worker: WorkerModel): number {
  const defaultAdding =
    worker.windowSpaces === WindowSpaces.custom
      ? worker.customWindowMinute
      : worker.windowSpaces === WindowSpaces.longestTreatment
 ...
 */
function overlapingDates(first: RecurrenceEvent, second: RecurrenceEvent): Set<Date> {
 ...
 */
function isOptionalTimeForBooking(
 ...
 */
function workerBookingOnMidnight(endWork: Date, booking: Booking): boolean {
 ...
 */
function isBlockFromMiddlePaymentUser(
  booking: Booking,
  timeToOrder: Date,
  date: string,
  worker: WorkerModel,
 ...
 */
function getOnlyEqualOrAfter(times: Date[], startTime: Date): Date[] {
 ...
 */
function getBookingTimes(booking: Booking): Date[] {
  const times: Date[] = [];
  const treatment = Treatment.fromTreatmentsMap(booking.treatments);
 ...
 */
function allFreeTimes(worker: WorkerModel, dateToCheck: Date): Date[] {
...
function getTimeDevideByFive(time: Date, inCheckFormat: boolean = false): Date {
...
function getFakeBookingFromTime(day: string, start: string, end: string, workerId: string = ""): Booking {
...
function isRightAfterEvent(event: Event, closestEvent: Event | null): boolean {
  if (closestEvent === null) {
    return false;
  }

  return closestEvent.to.getTime() - event.from.getTime() < 10 * 60 * 1000 &&
...
function dateToStrAccoridngToFormat(date: Date, format: string): string {
  return new Intl.DateTimeFormat(LanguageHelper().currentLaguageCode, { hour: "numeric", minute: "numeric" }).format(date);
}
...
function dateToCustomDayAndMonthStr(date: Date): string {
  return new Intl.DateTimeFormat("en-US", { day: "2-digit", month: "numeric" }).format(date);
}

function dateToCustomDayAndMonthAndHourStr(date: Date, showHours: boolean): string {
...
function timeStrToDate(date: string): Date {
  return new Date(`1970-01-01T${date}`);
}

function dateStrToDate(date: string): Date {
  return new Date(date);
}

function monthStrToDate(date: string): Date {
  return new Date(date);
}

function conversStrToDateTime(day: Date, strTime: string): Date {
...
function convers1970DateTimeToDay(day: Date, dateFrom1970: Date): Date {
  return new Date(day.getFullYear(), day.getMonth(), day.getDate(), dateFrom1970.getHours(), dateFrom1970.getMinutes());
}

function laterDate(date1: Date | null, date2: Date | null): Date | null {
...
function firstDate(date1: Date | null, date2: Date | null): Date | null {
  if (date1 === null && date2 === null) {
    return null;
  }
  if (date1 === null) {
...
function getFakeBookingFromBreak(breakModel: BreakModel): Booking {
  const treatment = new Treatment({
...
function setToStartOfYear(date: Date): Date {
  return new Date(date.getFullYear(), 0, 1);
}

function setToStartOfMonth(date: Date): Date {
...
function setTo1970(date: Date): Date {
...
function durationStrikings(start1: Date, end1: Date, start2: Date, end2: Date): string {
  if (start1.getTime() < start2.getTime() && end1.getTime() < start2.getTime()) {
    return 'NO_STRIKE';
  } else if (start1.getTime() > end2.getTime() && end1.getTime() > end2.getTime()) {
    return 'NO_STRIKE';
  } else {
    return 'STRIKE';
  }
...
function convertStringToTime(times: string[]): Date[] {
 ...
 */
function findOverlapingEvent(worker: WorkerModel, start: Date, end: Date): Date {
  ...
  worker.recurrence.recurrenceEvents.forEach((day, dayEvents) => {
    dayEvents.forEach((time, event) => {
      if (event.recurrenceEvent !== null &&
          event.recurrenceEvent.isAccureInDate({ date: start })) {
        const eventStart = new Date(start.getFullYear(), start.getMonth(), start.getDate(),
          event.from.hour, event.from.minute);
        const eventEnd = new Date(start.getFullYear(), start.getMonth(), start.getDate(),
          event.to.hour, event.to.minute);
  ...
  if (worker.events.allEvents[month] !== null &&
  ...
  const events = [...todayEvents, ...eventsFromRecurrence, ...Object.values(worker.events.breakEvents)];
  events = events.filter((event) => dateToDateStr(event.from) === dateToDateStr(start));
  ...
  for (const event of events) {
    ...
    if ((isRightAfterEvent(event, closestToStartEvent) ||
