  ...
  class FirebaseAuthService {
      ...
      private async signInWithFacebookWebSRV(loginType: LoginType): Promise<void> {
          ...
          switch (loginType) {
              ...
              case LoginType.reauthenticate:
                  await reauthenticateWithPopup(this._auth.currentUser!,facebookProvider);
                  break;
      ...
      async signInWithOtpSRV({
          loginType, verificationId, otp, autocredential
      }: {
          loginType: LoginType;
          ...
          const credential = autocredential ?? PhoneAuthProvider.credential(verificationId, otp);
          ...
          try {
              switch (loginType) {
                  ...
                  case LoginType.link:
                      ...
                      break;
                  case LoginType.reauthenticate:
                      await reauthenticateWithCredential(this._auth.currentUser!,credential);
                      break;
                  case LoginType.phoneUpdate:
    ...
    //   }
      ...
      get userPhoneSRV(): string {
          if (this._auth.currentUser !== null) {
              ...
              const phone = this._auth.currentUser.phoneNumber!.replace(prefix, `${prefix}-`);
              return phone;
          }
          return '';
      }
      ...
      async unlinkProviderSRV({ providerId }: { providerId: string; }): Promise<boolean> {
          try {
