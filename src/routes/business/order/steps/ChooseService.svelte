<script lang="ts">
  ...
  export let selectedServices: Array<Treatment>;
  ...
  function serviceChosen(treatment: Treatment, ss: Array<Treatment>) {
    if (selectMultiple) {
      BookingController.onTapService(treatment);
      if (!ss.includes(treatment)) {
        selectedServices = addService(treatment, ss);
      } else {
        while (selectedServices.includes(treatment)) {
          selectedServices = removeService(treatment, ss);
        }
      }
    } else {
      continuetoNextStep();
  ...
  function addService(
    service: Treatment,
    selectedServices: Array<Treatment>
  ): Array<Treatment> {
